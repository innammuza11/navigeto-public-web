  ...
  return <>
    ...
    <div className="shell content-wrap detail-grid">
      <div className="detail-main">
        ...
        {related.length ? <div className="content-section"><div className="eyebrow">You may also like</div><h2>Related programs</h2><div className="card-grid" style={{gridTemplateColumns:"repeat(3,1fr)"}}>{related.map((item)=><article className="tour-card" key={item.id}><div className="tour-art" style={item.hero_image_url ? { backgroundImage:`url(${item.hero_image_url})`, backgroundSize:"cover", backgroundPosition:"center" } : undefined}><span className="duration">{item.duration_nights} nights</span>{!item.hero_image_url ? <span className="tour-emoji">🌴</span> : null}</div><div className="tour-body"><h3>{item.title}</h3><div className="tour-bottom"><strong>{formatMoney(item.price_from,item.currency||"USD")}</strong><Link className="text-link" href={`/tours/${item.slug}`}>View →</Link></div></div></article>)}</div></div> : null}
      </div>
