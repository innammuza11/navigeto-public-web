...
type HotelForm = { q: string; checkin: string; checkout: string; rooms: string; guests: string; occupancy: string; meal_plan: string; market: string };

function HotelsContent() {
  const params = useSearchParams();
  const [form, setForm] = useState<HotelForm>({
    q: params.get("q") || "", checkin: params.get("checkin") || "", checkout: params.get("checkout") || "",
    rooms: params.get("rooms") || "1", guests: params.get("guests") || "2", occupancy: "double", meal_plan: "", market: "All Markets",
  });
  ...
  useEffect(() => {
    ...
    searchHotels({
      ...
      occupancy: "double", meal_plan: "", market: "All Markets", max_results: 100,
  ...
  return <>
    ...
    <div className="shell content-wrap">
      ...
      <div className="results-header"><div><h2>Available hotel rates</h2><p>Final customer selling rates for the selected stay.</p></div><b>{rows.length} result{rows.length === 1 ? "" : "s"}</b></div>
      {loading ? <LoadingCards count={6} /> : rows.length ? <div className="card-grid">{rows.map((row) => <article className="hotel-card" key={row.rate_id}>
...
function HotelBookingModal({ rate, search, onClose }: { rate: HotelResult; search: HotelForm; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><div className="modal"><div className="modal-head"><div><h2>{done ? "Request received" : `Request ${rate.hotel_name}`}</h2><p>{rate.room_type} · {rate.meal_plan} · {formatMoney(rate.total_amount, rate.currency)}</p></div><button className="close-button" onClick={onClose}>✕</button></div><div className="modal-body">
    {done ? <div className="reference-box"><span>Your TravelOS reference</span><strong>{done.public_ref}</strong><p>Our team will confirm availability and the next payment step.</p></div> : <form id="hotel-booking" className="form-grid two" onSubmit={submit}>
