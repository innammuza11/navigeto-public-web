...
export function HotelResults(){
 ...
 useEffect(()=>{const p=new URLSearchParams(window.location.search);const currentSearch=p.toString();const start=p.get("checkin")||"2026-08-15";const end=p.get("checkout")||"2026-08-19";liveApi.hotels({q:p.get("q")||"",checkin:start,checkout:end,rooms:Number(p.get("rooms")||1),occupancy:p.get("occupancy")||"double",adults:Number(p.get("adults")||2),children:Number(p.get("children")||0),meal_plan:p.get("meal_plan")==="any"?undefined:p.get("meal_plan")||undefined,market:p.get("market")||"All Markets",max_results:100}).then(r=>{setSearch(currentSearch);setRates(r.results);}).catch(e=>setError(e.message)).finally(()=>setLoading(false));},[]);
 const choose=(rate:HotelRate)=>{const p=new URLSearchParams(window.location.search);const checkin=p.get("checkin")||"2026-08-15";const checkout=p.get("checkout")||"2026-08-19";saveSelection("hotel",{...rate,checkin,checkout});window.location.assign("/hotels/booking");};
...
export function HotelDetail({slug}:{slug:string}){
 ...
 const chooseLive=(rate:HotelRate)=>saveSelection("hotel",{...rate,checkin:query.checkin,checkout:query.checkout});
