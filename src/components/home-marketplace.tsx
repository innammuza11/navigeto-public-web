"use client";
import { publicTourSummary, tourDisplayName, tourDurationLabel } from "@/lib/tour-presentation";

import Link from "next/link";
import { useEffect, useMemo, useState } from "react";
import { Money } from "@/components/money";
import { catalogueSection } from "@/lib/catalogue-section";
...
export function HomeMarketplace() {
  const [loading, setLoading] = useState({ hotels: true, tours: true, visas: true });
  ...
  useEffect(() => {
    let active = true;
    void catalogueSection(liveApi.hotelStartingRates({ max_results: 5 }).then(r => r.results.slice(0, 5)), []).then(hotels => {
      if (!active) return;
      setState(current => ({ ...current, hotels }));
      setLoading(current => ({ ...current, hotels: false }));
    });
    void catalogueSection(liveApi.tours({}).then(r => r.results), []).then(tours => {
      if (!active) return;
      setState(current => ({ ...current, internationalTours: tours.filter(tour => !isSriLanka(tour)).slice(0, 5), sriLankaTours: tours.filter(isSriLanka).slice(0, 5) }));
      setLoading(current => ({ ...current, tours: false }));
    });
    void catalogueSection(fetch("/api/visa/catalog", { signal: AbortSignal.timeout(12000) }).then(response => response.ok ? response.json() : Promise.reject(new Error("Visa catalogue unavailable"))).then(visaProducts), []).then(visas => {
      if (!active) return;
      ...
      setState(current => ({ ...current, visas: sortedVisas.slice(0, 5) }));
      setLoading(current => ({ ...current, visas: false }));
    });
    return () => { active = false; };
  }, []);
  ...
  return <section className="marketplace-section" aria-label="Navigeto travel marketplace">
    <div className="shell marketplace-intro">
      ...
      <p>Find a stay, explore a journey or plan your next getaway. Our team confirms prices and availability for your dates.</p>
    </div>

    <div className="shell marketplace-row">
      <MarketplaceHeading eyebrow="Featured hotels" title="Stay somewhere worth remembering." copy="Featured stays with starting rates to help you plan." href="/hotels" action="See all hotels"/>
      {loading.hotels ? <LoadingCards/> : state.hotels.length ? <div className="marketplace-grid">
      ...
      </div> : <EmptyMarketplace title="Hotel rates are being refreshed." copy="Search your dates and destination for the latest availability." href="/hotels/search" action="Search hotels"/>}
    </div>

    <div className="shell marketplace-row">
      <MarketplaceHeading eyebrow="International tours" title="The world, planned from Colombo." copy="Explore international journeys or ask us to plan a trip around your dates." href="/tours/international" action="Explore international tours"/>
      {loading.tours ? <LoadingCards/> : state.internationalTours.length ? <div className="marketplace-grid">
...
<div className="market-card-body"><h3 title={tour.title}>{tourDisplayName(tour)}</h3><p>{tour.destinations.slice(0, 3).join(" · ") || publicTourSummary(tour.summary, tour.destinations, tour.duration_days)}</p>{tour.price_from ? <Money value={tour.price_from} currency={tour.currency} suffix="Starting from"/> : <small>Price confirmed for your dates</small>}<Link href={`/tours/package/${tour.slug}`}>View journey <span aria-hidden="true">→</span></Link></div>
        </article>)}
    ...
    <div className="shell marketplace-row">
      <MarketplaceHeading eyebrow="Sri Lanka tours" title="Island journeys, ready to make your own." copy="Explore routes, daily itineraries and starting prices for your Sri Lanka holiday." href="/tours/sri-lanka" action="See all Sri Lanka tours"/>
      {loading.tours ? <LoadingCards/> : state.sriLankaTours.length ? <div className="marketplace-grid">
        {state.sriLankaTours.map((tour, index) => <article className="market-card market-tour-card" key={tour.id}>
          ...
          <div className="market-card-body"><h3 title={tour.title}>{tourDisplayName(tour)}</h3><p>{tour.destinations.slice(0, 4).join(" · ") || publicTourSummary(tour.summary, tour.destinations, tour.duration_days)}</p>{tour.price_from ? <Money value={tour.price_from} currency={tour.currency} suffix="Starting from"/> : <small>Price confirmed for your dates</small>}<Link href={`/tours/package/${tour.slug}`}>View itinerary <span aria-hidden="true">→</span></Link></div>
        </article>)}
      </div> : <EmptyMarketplace title="Sri Lanka tour inventory is being refreshed." copy="Tell us the route, dates and travellers and the team will prepare a personalised quote." href="/custom-trip" action="Build a Sri Lanka trip"/>}
    </div>

    <div className="shell marketplace-row">
      <MarketplaceHeading eyebrow="Visa services" title="Know the route before you apply." copy="Explore visa assistance, estimated fees and the next steps for your destination." href="/visas" action="See all Visa services"/>
      {loading.visas ? <LoadingCards/> : state.visas.length ? <div className="marketplace-grid">
