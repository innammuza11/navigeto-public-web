...
export function ProductPage({data,kind}:{data:Data;kind:string}) {
  ...
  return <>
    ...
    <section className="section pale"><div className="shell split"><div><p className="eyebrow">Why Navigeto Travels</p><h2>Modern tools behind the scenes. Real people when it matters.</h2><p>Our Sri Lanka team helps you choose the right route, understand your options and plan with confidence.</p></div><div className="check-list">{["Prices explained before you book","Clear inclusions and next steps","Sri Lanka-based support","Personal help with your travel plans"].map(x=><div key={x}><span>✓</span>{x}</div>)}</div></div></section>
