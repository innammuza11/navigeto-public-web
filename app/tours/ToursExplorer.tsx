...
function ToursContent({ initialTours, initialError }: { initialTours: PublicPackage[]; initialError?: string }) {
  const params = useSearchParams();
  const [rows] = useState<PublicPackage[]>(initialTours); const [error] = useState(initialError || "");
  ...
  return <>
    <PageHero image="/headers/tours.webp" eyebrow="Sri Lanka Tour Packages" title="Start with a proven route. Personalise every detail." description="Choose a published Navigeto program, then adjust hotels, pace, activities, meal plan and transport for your travellers." />
    <div className="shell content-wrap">
      ...
      {filtered.length ? <div className="card-grid">{filtered.map((tour) => <article className="tour-card" key={tour.id}>
...
export default function ToursExplorer({ initialTours, initialError }: { initialTours: PublicPackage[]; initialError?: string }) {return <Suspense fallback={<LoadingCards count={3}/>}><ToursContent initialTours={initialTours} initialError={initialError}/></Suspense>}
