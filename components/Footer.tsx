...
export function Footer() {
  ...
  return <footer className="footer">
    <div className="shell footer-grid">
      <div>
        <div className="brand footer-brand"><span className="brand-mark">N</span><span><strong>NAVIGETO</strong><small>TRAVELS · SRI LANKA</small></span></div>
        <p>{config.tagline}</p>
      ...
      <div><h4>Book with us</h4>{config.hotel_enabled?<Link href="/hotels">Hotels</Link>:null}{config.transfer_enabled?<Link href="/transfers">Private Transfers</Link>:null}{config.tour_enabled?<Link href="/tours">Sri Lanka Tours</Link>:null}<Link href="/custom-trip">Custom Trip</Link></div>
